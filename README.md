...
基于 LangChain + Chroma 的电商商品咨询检索增强生成（RAG）问答系统，支持多轮对话与知识库在线更新，覆盖尺码、颜色、洗涤养护等高频咨询。
...
├── compare.py               # 对照实验：同一问题无 RAG vs 有 RAG 对比输出
...
同一个问题，分别裸问 DeepSeek 和走 RAG 链两条路径回答，直观对比检索增强的效果——无 RAG 只有通用常识，有 RAG 答案来自知识库、有据可依。运行 `python compare.py` 可复现。
